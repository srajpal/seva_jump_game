...
(async () => {
  ...
  assert.equal(cacheName, `seva-jump-${encodeURIComponent('/html/123456/')}-v1.0.0`);
  ...
  const scriptResponse = await worker.dispatch('fetch', request(`${worker.scope}game.js?v=1.0.0`, 'script'));
  ...
  const oldName = cacheName.replace('v1.0.0', 'v0.13.1');
