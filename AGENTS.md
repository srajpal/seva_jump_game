...
SevaJump is a free, offline-first, touch-friendly vertical jumping game for ages 8-15. The browser game is the source of truth; Capacitor packages it as the portrait-only Android app, and `ios/` contains a native iOS wrapper. Preserve the no ads, purchases, accounts, analytics, and gameplay-network-dependency model.
...
Read `README.md` for game behaviour/build context. Use `RELEASE_CHECKLIST.md` for release validation and `STORE_LISTING.md` for Play Store copy and declarations. `ITCH_RELEASE_AUDIT.md` records the September 2026 browser audit and unresolved release findings; it is a dated assessment, not a permanent certification. Inspect only the files relevant to the task; avoid unrelated refactors or feature expansion.
...
node tests\runtime-browser-checks.js
node tests\gameplay-runtime-checks.js
node tests\service-worker-checks.js
...
`npm run test:browser` requires Playwright and an installed test browser; see `TESTING.md`. `npm run itch:build` produces the curated ZIP and checksums in ignored `dist/`. Never include test hooks in shipped code: runtime tests inject their inspection seam only into an in-memory copy.
